...
//MEDICACION
document.getElementById('btnConfirmarFechas').addEventListener('click', function () {
    const mediIdPaciente = document.getElementById('id').value; // Obtener el ID del paciente
    const fechaDesde = document.getElementById('fechaDesde').value; // Obtener fecha desde
    const fechaHasta = document.getElementById('fechaHasta').value; // Obtener fecha hasta
    ...
    // Realizar la consulta AJAX al archivo PHP
    const xhr = new XMLHttpRequest();
    xhr.open('GET', `./dato/get_medicaciones.php?id_paciente=${mediIdPaciente}&fecha_desde=${fechaDesde}&fecha_hasta=${fechaHasta}`, true);
    xhr.onload = function () {
        if (this.status === 200) {
            const datosMedicacion = JSON.parse(this.responseText);

            // Generar el PDF
            const { jsPDF } = window.jspdf;
            const doc = new jsPDF();

            // Establecer el tamaño y el título
            doc.setFontSize(18);
            doc.text('Reporte de Medicación del paciente', 14, 22);
            doc.text(datosMedicacion[0].nombre_paciente, 14, 30); // Cambia la coordenada Y para colocarlo debajo
            doc.setFontSize(12);
            doc.text(`Desde: ${formatDate(fechaDesde)}  Hasta: ${formatDate(fechaHasta)}`, 14, 38);

            // Calcular la suma total de los valores
            const totalValor = datosMedicacion.reduce((total, item) => total + (parseFloat(item.valor) || 0), 0);

            // Generar la tabla con las nuevas columnas
            doc.autoTable({
                head: [['Medicamento', 'Fecha', 'Hora', 'Dosis', 'Cantidad', 'Valor']],
                body: datosMedicacion.map(item => [
                    item.desc_medi,
            ...
            // Obtener la posición final de la tabla para colocar la imagen y el total
            const finalY = doc.autoTable.previous.finalY;

            // Agregar el total de "Valor general" al final
            doc.setFontSize(12);
            doc.text(`Valor general: ${totalValor.toFixed(2)}`, 14, finalY + 10);

            // Agregar el logo centrado después de la tabla y del total
            ...
            img.onload = function () {
                ...
                const pageWidth = doc.internal.pageSize.getWidth(); // Ancho de la página
                const xImg = (pageWidth - imgWidth) / 2; // Calcular la posición X para centrar
                const yImg = finalY + 20; // La posición Y justo debajo de la tabla
    ...
    $(document).on('click', '#editMedi', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#cantidad').val(medi.cantidad);
                $('#valor').val(medi.valor);
                $('#mediIdPaciente').val(medi.id_paciente);
                $('#mediNombreCarga').val(medi.nombre_paciente);

                // Concatenar descripcion y potencia para el campo de búsqueda
                const optionText = `${medi.descripcion}`;

                // Rellenar el campo de búsqueda con la descripción del medicamento
                $('#searchMedicacion').val(optionText).trigger('input'); // Esto activará la carga de `cargarMedicacion`

                // Crear un observador para detectar cambios en el select
                const mediDescSelect = document.getElementById('mediDesc');
                const observer = new MutationObserver(function () {
                    const optionExists = $('#mediDesc').find(`option[value="${medi.medicamento}"]`).length > 0;

                    if (optionExists) {
                        $('#mediDesc').val(medi.medicamento); // Seleccionar el medicamento actual
                        observer.disconnect(); // Desactivar el observador después de encontrar la coincidencia
                    }
                });

                // Configurar el observador para observar cambios en el contenido del select
                observer.observe(mediDescSelect, { childList: true });
                // Establecer data-action a "edit"
...
function loadPagosPaciente() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#pagoPacienteModal').modal('show'); // Mostrar el modal de egreso
            $('#formPaciente').hide(); // Ocultar el formulario principal usando jQuery al cargar el modal
        },
        error: function (jqXHR, textStatus, errorThrown) {
            console.log('Error en loadPracticasModal:', textStatus, errorThrown);
...
// Función para mostrar el modal de agregar/editar paciente al hacer clic en "Volver" dentro del modal de egreso
$('#pagoPacienteModal').on('click', '.btn-volver', function () {
    $('#formPaciente').show(); // Mostrar el formulario principal al hacer clic en "Volver"
    $('#pagoPacienteModal').modal('hide'); // Ocultar el modal de egreso
...
// Función para cargar la lista de egresos desde la base de datos
function cargarListaPagos(idPaciente, page = 1, recordsPerPage = 100) {
    $.ajax({
        url: './dato/get_pagos.php',
        type: 'GET',
        data: {
            id_paciente: idPaciente,
            page: page,
        ...
        success: function (response) {
            ...
            pagos.forEach(function (pago) {
                html += '<tr>';
                html += '<td>' + formatDate(pago.fecha_pago) + '</td>';
                html += '<td>' + pago.profesional_full + '</td>';
                html += '<td>' + pago.actividad_full + '</td>';
                html += '<td>' + pago.monto_pago + '</td>';
                html += '<td>' + pago.monto_total + '</td>';
                html += '<td>' + pago.metodo_pago + '</td>';
                html += '<td>' + pago.obs + '</td>';
                html += '<td>' + pago.deuda + '</td>';
                html += '<td>';
                html += '<button id="editPago" class="btn btn-primary btn-custom-save btn-sm btn-edit" data-id="' + pago.id_pago + '">Editar</button> ';
                html += '<button id="deletePago" class="btn btn-danger btn-sm btn-delete" data-id="' + pago.id_pago + '">Eliminar</button>';
                html += '</td>';
                html += '</tr>';
            });
            ...
            $('#listaPagos').html(html); // Insertar el HTML generado en el contenedor
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            console.log('Error en cargarListaPagos:', textStatus, errorThrown);
        }
    });
}
...
$(document).on('change', '#recordsPerPage', function () {
    var recordsPerPage = $(this).val();
    const idPaciente = document.getElementById('id').value;
    cargarListaPagos(idPaciente, 1, recordsPerPage); // Recargar la tabla con la nueva cantidad de registros por página
});
...
$(document).on('click', '#pagination .page-link', function (e) {
    ...
    var recordsPerPage = $('#recordsPerPage').val();
    const idPaciente = document.getElementById('id').value;
    cargarListaPagos(idPaciente, page, recordsPerPage); // Recargar la tabla con la página seleccionada
});


// Evento al mostrar el modal de egresos
$('#pagoPacienteModal').on('shown.bs.modal', function () {
    const idPaciente = document.getElementById('id').value; // Obtener el ID del paciente desde un campo oculto en el formulario
    cargarListaPagos(idPaciente); // Cargar la lista de egresos al mostrar el modal
});

// Función para mostrar nuevamente el formulario principal al cerrar el modal de egreso
$('#pagoPacienteModal').on('hidden.bs.modal', function () {
    $('#formPaciente').show(); // Mostrar el formulario principal al cerrar el modal de egreso
});

$(document).ready(function () {
    // Mostrar el modal de agregar práctica al hacer clic en el botón "Agregar"
    $('#nuevoPago').on('click', function () {
        console.log('Botón "Agregar" clickeado');
        const id = $('#id').val();
        const nombre = $('#nombre').val();
        ...
        $('#pagoCantidad').val('');
        $('#pagoTotal').val('');
        $('#pagoIdPaciente').val(id);
        $('#pagoNombreCarga').val(nombre);

        // Establecer data-action a "add"
        $('#btnGuardarPago').attr('data-action', 'add');
    ...
    // Mostrar el modal de editar práctica al hacer clic en el botón "Editar"
    $(document).on('click', '#editPago', function () {
        var pracId = $(this).data('id');

        $.ajax({
            url: './dato/get_pago_con_id.php',
            type: 'GET',
            data: { id: pracId },
            success: function (response) {
                ...
                // Llenar los campos del modal de edición con los datos de la práctica
                $('#pagoId').val(pago.id_pago);
                $('#pagoIdPaciente').val(pago.id_paciente);
                $('#pagoNombreCarga').val(pago.nombre);
                $('#pagoFechas').val(pago.fecha_pago);
                $('#pagoProfesional').val(pago.id_profesional);
                $('#pagoActividad').val(pago.actividad);
                $('#pagoCantidad').val(pago.monto_pago);
                $('#pagoTotal').val(pago.monto_total);
                $('#pagoMetodo').val(pago.metodo_pago);
                $('#pagoObs').val(pago.obs);

                // Establecer data-action a "edit"
                $('#btnGuardarPago').attr('data-action', 'edit');
    ...
    $('#btnGuardarPago').on('click', function () {
        var formData = $('#formAgregarPago').serializeArray();
        var action = $(this).attr('data-action');
        var url = action === 'edit' ? './submenu/pagos_pacientes/editar_pago.php' : './submenu/pagos_pacientes/agregar_pago.php';
    ...
    $(document).on('click', '#deletePago', function () {
        ...
        if (confirm('¿Estás seguro de que deseas eliminar este pago?')) {
            $.ajax({
                url: './submenu/pagos_pacientes/borrar_pago.php',
                type: 'POST',
                data: { id: pracId },
                success: function (response) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('btnGenerarPDFPago').addEventListener('click', function () {
        $('#modalFechasPDF').modal('show');
    });

    // Generar PDF al confirmar las fechas
    document.getElementById('btnConfirmarPDF').addEventListener('click', async function () {
        const fechaDesde = document.getElementById('fechaDesdePago').value;
        const fechaHasta = document.getElementById('fechaHastaPago').value;
        const idPaciente = document.getElementById('id').value;
        ...
        try {
            // Obtener datos de pagos del paciente dentro del rango de fechas
            const response = await fetch(`./submenu/pagos_pacientes/get_pagos_rango.php?idPaciente=${idPaciente}&fechaDesde=${fechaDesde}&fechaHasta=${fechaHasta}`, {
                method: 'GET',  // Usar GET ya que los parámetros van en la URL
                headers: { 'Content-Type': 'application/json' }  // El contenido sigue siendo JSON
            });
            ...
            // Encabezado
            const nombrePaci = document.getElementById('pagoNombre').value
            doc.setFontSize(16);
            doc.text(`Pagos del Paciente: ${nombrePaci}`, 10, 10);
            doc.setFontSize(12);
            doc.text(`Rango de Fechas: ${formatDate(fechaDesde)} - ${formatDate(fechaHasta)}`, 10, 20);

            // Tabla con los pagos
            doc.autoTable({
                head: [['Fecha', 'Profesional', 'Actividad', 'Monto Pagado', 'Monto Total', 'Método', 'Observaciones']],
                body: data.pagos.map(pago => [
                ...
                startY: 35,
                theme: 'striped',  // Añadir un tema para mejorar la apariencia
                margin: { top: 30 },  // Márgenes para evitar que el contenido quede cortado
                bodyStyles: { fontSize: 10 },  // Ajustar tamaño de la fuente
                tableWidth: 'auto',  // Ajustar automáticamente el ancho de las celdas
                columnStyles: { 0: { cellWidth: 'auto' }, 1: { cellWidth: 'auto' }, 2: { cellWidth: 'auto' }, 3: { cellWidth: 'auto' }, 4: { cellWidth: 'auto' }, 5: { cellWidth: 'auto' }, 6: { cellWidth: 'auto' } },
            });
