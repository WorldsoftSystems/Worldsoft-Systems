...
if ($result->num_rows > 0) {
    ...
    $linea1 = "CABECERA\n";
    $linea2 = "30-65642500-4;;$fechaActual;$periodo;$inst;2;UP30656425004;69634\n";
    ...
    $sqlBenefs = "SELECT DISTINCT p.*
                 ...
                 WHERE  (practs.fecha BETWEEN '$fechaInicio' AND '$fechaFin') AND p.obra_social = 4 AND p.ugl_paciente = 35 AND act.modalidad NOT IN (11,12,13)
                 ";
    ...
    $sqlAmbulatorioPsi = "WITH ValidRecords AS (
        ...
        WHERE pract.fecha BETWEEN '$fechaInicio' AND '$fechaFin'
          AND p.obra_social = 4  AND p.ugl_paciente = 35
    )
    ...
    $response = array(
        'filename' => $c_interno . "_UGL_35_AMB_WSS.txt",  // Nombre dinámico del archivo
        'content' => $contenido
    );
