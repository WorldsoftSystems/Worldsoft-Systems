...
document.getElementById('btnConfirmarFechas').addEventListener('click', function () {
    ...
    xhr.onload = function () {
        if (this.status === 200) {
            ...
            doc.setFontSize(18);
            doc.text('Reporte de Medicación', 14, 22);

            // Generar la tabla
            doc.autoTable({
                head: [['Paciente', 'Medicamento', 'Fecha', 'Hora', 'Dosis']],
                body: datosMedicacion.map(item => [item.nombre_paciente, item.desc_medi, formatDate(item.fecha), item.hora, item.dosis]),
                startY: 30,
            });

            // Obtener la posición final de la tabla para colocar la imagen
