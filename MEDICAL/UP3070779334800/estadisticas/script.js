...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function generatePacientesBocaPDF(fechaDesde, fechaHasta, obraSocialId) {
        ...
        Promise.all([fetchPaciBoca(fechaDesde, fechaHasta, obraSocialId)]).then(([resumen]) => {
            ...
            // Agrupar por boca de atención
            const groupedByBoca = resumen.reduce((acc, item) => {
                const boca = item.boca_de_atencion;
                const normalizedNombre = normalizeName(item.nombre);

                if (!acc[boca]) {
                    acc[boca] = { data: [], total: 0 };
                }
            ...
            console.log('Datos agrupados por boca:', groupedByBoca); // Verifica la agrupación aquí

            // Encabezado de la tabla
            const headers = ['AFILIADO', 'BENEFICIO', 'INGRESO', 'ULT. ATENCION', 'EGRESO', 'MODALIDAD', 'DIAGNOSTICO'];

            let totalRegistros = 0;

            // Iterar sobre los datos agrupados por boca de atención
            for (const boca in groupedByBoca) {
                startY += 10;

                // Agregar título de boca de atención
                doc.setFontSize(14);
                doc.text(`BOCA DE ATENCION: ${boca}`, pageWidth / 2, startY, { align: 'center' });
                startY += 5;

                const { data, total } = groupedByBoca[boca];
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    // Actualiza `startY` después de generar la tabla
                    const finalY = startY + data.length * 10 + 10; // Calcula la posición final
                    startY = finalY; // Actualiza `startY`

                    // Mostrar total por boca
                    doc.setFontSize(12);
                    doc.text(`TOTAL PACIENTES PARA BOCA DE ATENCION: ${total}`, pageWidth / 2, startY, { align: 'center' });
                    startY += 10;
                }
            }
                ...
                if (modalities.length > 0) {
                    ...
                    modalities.forEach(modality => {
                        ...
                        if (data.length) {
                            ...
                            doc.setFontSize(12);
                            doc.text(`Total Cantidad ${modalityUpperCase}: ${totalQuantity}`, pageWidth / 2, tableStartY + 10, { align: 'center' });
